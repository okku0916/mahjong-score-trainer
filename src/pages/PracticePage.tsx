...
export function PracticePage() {
  ...
  const mode = (
    raw === "fu" || raw === "han" || raw === "score" ? raw : null
  ) as Mode | null;
  const [q, setQ] = useState<Question | null>(null);
  const [selected, setSelected] = useState<string | null>(null);
  useEffect(() => {
    if (mode) {
      setQ(createQuestion(mode));
      setSelected(null);
    }
  }, [mode]);
  if (!mode)
  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.problem}>
        ...
        {q.tiles && (
          <div className={styles.hand} aria-label="手牌">
            {q.tiles.map((t, i) => (
              <img key={`${t}-${i}`} src={tileSrc(t)} alt={tileName(t)} />
            ))}
            <div className={styles.win}>
              <small>和了牌</small>
              <img
                src={tileSrc(q.winTile!)}
        ...
        <div className={styles.choices}>
          {q.choices.map((c) => {
            ...
            return (
              ...
              >
                {c}
                {answered && c === q.answer && <span>✓</span>}
                {answered && c === selected && c !== q.answer && <span>×</span>}
              </button>
            );
          })}
        </div>
      ...
      {answered && (
        <section
          className={`${styles.result} ${correct ? styles.ok : styles.ng}`}
          aria-live="polite"
        >
          <div className={styles.verdict}>
            <span>{correct ? "✓" : "×"}</span>
            <div>
              <small>{correct ? "正解です！" : "おしい！"}</small>
              <h2>正解は {q.answer}</h2>
            </div>
          </div>
          <div className={styles.explanation}>
            <h3>計算の内訳</h3>
            {q.explanation.map((x, i) => (
              <p key={x}>
                <b>{i + 1}</b>
                {x}
              </p>
